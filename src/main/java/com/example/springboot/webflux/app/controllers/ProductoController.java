...
public class ProductoController {
	...
	/*
	 * @GetMapping({ "/listar", "/" }) public String listar(Model model) { /*
	 * Flux<Producto> productos = service.findAll().map(producto -> {
	 * producto.setNombre(producto.getNombre().toUpperCase()); return producto; });
	 */
	/*
	 ...
	 * 
	 * model.addAttribute("productos", productos); model.addAttribute("titulo",
	 * "Listado de productos"); return "listar"; }
	 */
	...
	@GetMapping("/form-v2/{id}") // es mas reactivo
	...
	@PostMapping("/form") // si el modelo se llama diferente al objeto usar @ModelAttribute("producto")
	...
	public Mono<String> guardar(@Valid Producto producto, BindingResult result, Model model, @RequestPart FilePart file,
		...
		if (result.hasErrors()) {
			...
			return categoria.flatMap(c -> {
				...
				log.info("Categoria asignada: " + p.getCategoria().getId() + " " + p.getCategoria().getNombre());
	...
	public Mono<String> eliminar(@PathVariable String id) {
		return service.findById(id).defaultIfEmpty(new Producto()).flatMap(p -> {
			...
			log.info("Eliminarndo Producto Id:" + p.getNombre());
